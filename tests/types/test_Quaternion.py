...
class QuaternionTests(TestCase):
    ...
    def test_RotationEqual(self):
        for testCollection in self.testCollections:

            for i in range(0, testCollection.quats.__len__()):
                rotations = []

                rotations.append(Rotation3D.fromQuat(Quaternion.fromList(testCollection.quats[i]),
                                               order=testCollection.order))
                rotations.append(Rotation3D.fromEuler(EulerAngles.fromList(testCollection.rpys[i],
                                                                     order=testCollection.order)))

                rotations.append(Rotation3D.fromRotationMatrix(RotationMatrix(testCollection.rotMs[i],
                                                                        order=testCollection.order)))

                rotations.append(Rotation3D.fromAxisAngles(AxisAngles.fromList(testCollection.axisAngles[i])))
                ...
                for res1 in results:
                    for res2 in results:
                        ...
                        self.assertTrue(equal, "RotMs are not equal!")
                ...
                for res1 in results:
                    for res2 in results:
                        ...
                        self.assertTrue(equal, "Quats are not equal!")
                ...
                for res1 in results:
                    for res2 in results:
                        ...
                        self.assertTrue(equal, "RPYs are not equal!")
                ...
                for res1 in results:
                    for res2 in results:
                        ...
                        self.assertTrue(equal, "AxisAngles are not equal!")
